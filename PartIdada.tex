...
\end{knitrout}
\begin{figure}
\includegraphics[width=0.48\maxwidth]{dadafigure/plot-rates-1} 
\includegraphics[width=0.48\maxwidth]{dadafigure/plot-rates-2} 
...
We used the *[phangorn](http://cran.fhcrc.org/web/packages/phangorn/index.html)* R package  to construct a
...
\hlstd{samdf} \hlkwb{<-} \hlstd{samdf[}\hlopt{!}\hlkwd{duplicated}\hlstd{(samdf}\hlopt{$}\hlstd{SampleID),]} \hlcom{# Remove dupicate entries for reverse reads}
...
\hlstd{keep.cols} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{"collection_date"}\hlstd{,} \hlstr{"biome"}\hlstd{,} \hlstr{"target_gene"}\hlstd{,} \hlstr{"target_subfragment"}\hlstd{,} \hlstr{"host_common_name"}\hlstd{,} \hlstr{"host_subject_id"}\hlstd{,} \hlstr{"age"}\hlstd{,} \hlstr{"sex"}\hlstd{,} \hlstr{"body_product"}\hlstd{,} \hlstr{"tot_mass"}\hlstd{,} \hlstr{"diet"}\hlstd{,} \hlstr{"family_relationship"}\hlstd{,} \hlstr{"genotype"}\hlstd{,} \hlstr{"SampleID"}\hlstd{)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{ps} \hlkwb{<-} \hlkwd{phyloseq}\hlstd{(}\hlkwd{tax_table}\hlstd{(taxtab),}
               \hlkwd{sample_data}\hlstd{(samdf),}
               \hlkwd{otu_table}\hlstd{(seqtab,} \hlkwc{taxa_are_rows} \hlstd{=} \hlnum{FALSE}\hlstd{),}
               \hlkwd{phy_tree}\hlstd{(fitGTR}\hlopt{$}\hlstd{tree))}
\end{alltt}
\end{kframe}
\end{knitrout}
