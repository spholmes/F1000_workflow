...
taxonomy, and creation of the phylogenetic tree relating the sample
sequences. 
...
base_dir <- "/Users/susan/Phyloseq/BiocWorklow"
miseq_path <- file.path(base_dir, "data", "MiSeq_SOP")
read_chunk(file.path(base_dir, "src", "bioinformatics.R"))
@ 

<<init>>=
@ 

The data we will process here are highly-overlapping Illumina Miseq
...
These data can be downloaded from the following location:
...
<<profile>>=
@ 
\caption{Forward Error Profiles}
...
After filtering, the typical amplicon bioinformatics workflows cluster
...
method to to infer sequence variants without any fixed threshhold, thereby 
...
name the resulting `derep-class` objects by their sample.

<<derep>>=
@ 

The DADA2 method relies on a parameterized model of substitution
errors in order to distinguish sequencing errors from real biological
...
points) and the fitted error rates (black lines).
...
Independent inference by sample (`pool=FALSE`), and pooled inference from the 
set of sequencing reads combined from all samples (`pool=TRUE`). Independent
inference has two major advantages: Computation time is linear
...
Sequence inference removed (nearly) all substitution and
indel errors from the data. We now merge together the inferred forward 
and reverse sequences, while removing paired sequences that do not perfectly
...
analogue to the common "OTU table", i.e. a sample by sequence feature
...
other sequences in the table, and removing those that can be reproduced by
stitching together two more abundant sequences.

<<chimeras>>=
@ 

Typically a substantial fraction of inferred sequence variants, but
only a small fraction of all reads, are found to be chimeric. That is
what is observed here: 1502 of 1892 sequence variants were chimeric, but
...
rRNA gene is the ability to taxonomically classify the sequenced
variants. The *[dada2](http://bioconductor.org/packages/dada2)*
...
variants to a training set of classified sequences. Here we use the
RDP v14 training set \cite{cole2009rdp}.

<<tax>>=
@ 

Training set {\tt fasta} formatted files for use with the
\href{http://bioconductor.org/packages/dada2}{dada2} package are
...
The \Sexpr{CRANpkg("phangorn")} R package is then used to construct a
phylogenetic tree. Here we first construct a neighbor-joining tree,
and then fit a GTR+G+I maximum likelihood tree using the
...
The \Sexpr{Biocpkg("phyloseq")} package
