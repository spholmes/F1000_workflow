...
The following ensures that features with ambiguous phylum annotation are removed.
...
# Plot prevalence
ggplot(prevdf1, aes(TotalAbundance, Prevalence / nsamples(ps0))) + 
  # Include a guess for parameter, to see where it false
  ...
  facet_wrap(~Phylum) +
  ggtitle("Prevalence (as frac. of samples), v. Total Counts")
@
...
that descend from the user-prescribed agglomerating rank.
...
p3tree = plot_tree(ps3,
                   method = "treeonly", 
                   ladderize = "left",
                   title = paste0("tax_glom(..., taxrank='", "Genus", "')")) +
  theme(plot.title = element_text(size = multiPlotTitleTextSize))
