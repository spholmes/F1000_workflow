...
In version 2 of the manuscript, we have updated the procedure for stocking the
filtered and trimmed files during the call to dada2 to avoid overwriting.
Replacing
{\tt msa} alignment function with
from the \BioCpkg{DECIPHER}\citep{DECIPHER} package.

We have expanded slightly the phyloseq section, reduced the number of network plots
and provided detailed discussion of our choice not to make the PCoA and PCA plots square.
